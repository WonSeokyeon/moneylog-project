...
| 19    | 챗봇 규칙 고도화 (규칙 기반 유지)            | frontend | ✅   |
...
| CHAT-06 예상 지출                        | 19(`stats.forecast` 그대로)                               | 19                                        |
| CHAT-07 평소 대비 이상치                 | 19(`stats.anomalies` 그대로)                              | 19                                        |
| CHAT-08 지출 상위 카테고리               | 19(`stats.byCategory`)                                    | 19                                        |
| CHAT-09 두 달 비교                       | 19(기준 달 통계 쿼리 추가)                                | 19                                        |
| CHAT-10 하루 지출                        | 19(`stats.daily`)                                         | 19                                        |
| CHAT-11 거래처 검색 + 합계               | 19(목록 `keyword` 재사용)                                 | 19                                        |
| CHAT-12 오타·띄어쓰기·월 해석            | 19(`lib/chat/matcher.ts`)                                 | 19(`npm run check:chat`)                  |
...
> **후속 변경(Phase 20)**: 위 세리프 항목은 폐기됐다. 제목 폰트는 Pretendard로 통일했고(`--font-heading: var(--font-sans)`), 액센트·수입 색은 WCAG AA 대비를 위해 라이트 `#B8542F` · `#166C40`으로 조정했다(`CLAUDE.md` 8장이 현재 값의 정본).
- [x] 카테고리 색 점 9종이 이전과 동일함(팔레트 변경 영향 없음)
- [x] `npx tsc --noEmit`, `npm run lint`, `npm run build` 통과

---

## Phase 19 — 챗봇 규칙 고도화 (규칙 기반 유지)

**저장소**: `moneylog-frontend` · **선행 조건**: Phase 13 · **관련 요구사항**: CHAT-06~12

LLM 연동도 검토했지만 **규칙 기반을 유지하기로 했다** — 영수증 인식에서 API 키 의존을 한 번 걷어낸 전례(Phase 14 철회)와 같은 이유다. 대신 동작 규칙을 `PRD.md` 5.1 「챗봇 규칙」(R1 입력 정규화 · R2 기간 해석 · R3 의도 우선순위 · R4 답변 규칙)에 **정본으로 먼저 적고** 코드를 그 표에 맞췄다. 월별 통계 응답에 이미 `forecast`·`anomalies`·`daily`·`byCategory`·`budgets`가 들어 있어 **새 백엔드 API 없이** 의도를 5종에서 13종으로 늘렸다.

**작업**

- `lib/chat/matcher.ts`(신규) — R1. 공백 무시 비교, **자모 단위 편집 거리 1** 오타 허용(1음절·비한글 키워드 제외), 다른 등록 단어 자리에는 오타 보정 금지(마스킹), 카테고리 이름의 `/` 조각·기호 무시·정확 일치 우선
- `lib/chat/intents.ts` — R2·R3. `parseIntent`의 분기 순서가 R3 표의 순위와 1:1. 신규 의도: 미래 안내·예상 지출·이상치·두 달 비교·지출 순위·거래처 검색·수입 카테고리·하루 지출
- `lib/chat/answers.ts` — R4. 첫 줄에 조회 범위, 이번 달 비교 시 "아직 N일째" 단서, 기준 0이면 비율 생략, 목록 "전체 N건 중 M건", 일부 합계는 "최근 M건 합계(전체 N건)"
- `ChatPanel.tsx` — 기준 달 통계 쿼리 추가(기본값 지난달), 목록 `keyword`·`totalElements` 전달, 필요한 조회가 실패하면 대기 메시지에서 멈추지 않고 실패 문구로 종료
- `lib/date.ts` — `shiftYearMonth` 추가
- `scripts/check-chat-rules.mjs` + `npm run check:chat` — R1~R3 회귀 검사 34건. 의존성 없이 실제 `parseIntent`를 Node 24로 실행한다

**DoD**

- [x] `npm run check:chat` 34건 전부 통과 — 특히 **"이번달 예상 지출"이 예산 질문으로 가지 않고**(예상/예산 자모 1개 차이), **"오는 길에"가 "오늘"로 읽히지 않으며**, **"엄마"가 "얼마"로 읽히지 않음**
- [x] "지난달보다 얼마나 더 썼어" → 이번 달 vs 지난달 비교 + "아직 N일째" 단서
- [x] "스타벅스에서 얼마 썼어" → 거래처 검색 목록 + 합계(전체인지 일부인지 명시) + 딥링크
- [x] "기타수입 얼마" → `기타`가 아니라 `기타수입`으로 매칭(긴 이름 우선)
- [x] 연도 없이 아직 오지 않은 달("10월", 9월 기준)을 물으면 작년 10월로 해석
- [x] `npx tsc --noEmit`, `npm run lint`, `npm run build` 통과
- [ ] 실제 브라우저에서 13종 의도를 한 번씩 물어 답변 문구 확인 — ⚠️ 미검증. 이 세션의 브라우저 도구가 다른 앱에 연결돼 있어 직접 확인하지 못했다. 파싱은 스크립트로, 답변 조립은 타입 검사로만 확인함

---

## Phase 20 — 접근성 · 일러스트 · 진입 모션 (디자인 개선)

**저장소**: `moneylog-frontend`(코드) · `moneylog-project`(문서) · **관련 요구사항**: `UX-09` 후속, 화면 디자인(`CLAUDE.md` 8장)

`frontenddesign.md`(접근성 · 위계 · 피드백 · 모바일 우선, 이후 대담한 미학·모션 지침)를 기준으로 화면을 점검·개선했다. **API 변경은 닉네임 변경 하나뿐이다**(아래 「닉네임 변경」).

**작업**

- 접근성: 명도 대비 AA(라이트 액센트 `#B8542F`, 수입 `#166C40`, 다크 경고 `#D4735C`, 다크 버튼 글자 `#141413`), 터치 기기에서 버튼·입력·탭·셀렉트 44px(`pointer-coarse:`), 본문 바로가기 링크, 메뉴 `aria-label`·`aria-current`, 예산 막대 `progressbar`, 라벨 없던 입력 3곳
- 위계·피드백: 화면별 `h1`(`PageHeader`), 대시보드 히어로에 주요 행동 하나(거래 기록하기), 저장 성공 토스트(예산·카테고리·거래), 글자 크기 +3px, 제목 폰트 Pretendard 통일
- 일러스트: `components/illustration/Art.tsx`에 SVG 7종(이미지 파일·`next/image` 없음), 빈 상태·에러 상태·페이지 제목·대시보드 히어로에 적용
- 로그인·회원가입: 2단 레이아웃(`(auth)/layout.tsx`) + `AuthShowcase`(그래프 그려짐 → 동전 → 도장)
- 모션: `.reveal-stack` 순차 등장(CSS), 요약 숫자 카운트업, 헤더 메뉴 알약 이동. 모두 `prefers-reduced-motion`이면 꺼짐
- 카테고리별 지출 차트 선택: 카드 제목 옆 탭으로 원형(도넛) · 막대 · 트리맵 전환(`CategoryBar`, `CategoryTreemap`을 `chart/`에 추가, 세 차트가 같은 `ChartDatum` props와 같은 색을 씀). 탭 이름은 도넛이 "원형"
- 챗봇 위젯: 아이콘을 끌어 화면 안에서 위치 이동(패널이 따라감, 가장자리 8px 제한), 브라우저 기본 우하단 `resize` 대신 좌상단 핸들로 크기 조절, 버튼 36px → 56px
- 대시보드 광고 배너: 큰 히어로를 인사말 한 줄로 줄이고, 그 자리에 자동 넘김 광고 배너 3장(`AdBannerCarousel`, `lib/banners.ts`, `public/assets/banners/*.svg`)을 임의 문구·이미지·주소로 넣었다(자리표시)
- 화면 다듬기: 모바일 로그인 배너에 헤드라인·칩 표시 + 폼 여백 축소, 종이 배경 그라데이션을 Clay 한 색으로(탁한 회색 제거), 거래 목록 날짜 한 줄 고정, 입력 폼 라벨 왼쪽 정렬, 달력 히트맵에 이전·다음 달 날짜를 옅게 표시, 대시보드 인사말을 무지출 챌린지 문구로 교체
- 닉네임 변경(`AUTH-10`): 헤더의 닉네임을 누르면 `NicknameDialog`가 열린다. 문서(`CLAUDE.md` 5장·`PRD.md`) → 백엔드 `PUT /api/v1/auth/me`(1~50자, `AuthService.updateNickname`) → 프론트엔드 순으로 반영했다
- 이번 점검에서 잡은 결함: 토스트가 헤더의 다크 토글을 무시하고 OS 설정만 따르던 것(`sonner.tsx`), `AuthShowcase`가 모션 감소 설정에서 서버 렌더와 다른 초기값을 그려 하이드레이션 불일치가 날 수 있던 것

**DoD**

- [x] `npx tsc --noEmit`, `npm run lint`, `npm run build` 통과 (`.next`를 개발 서버와 공유하지 않도록 임시 복사본에서 빌드)
- [x] `npm run check:chat` 34건 통과
- [x] 백엔드 `./mvnw test` 86건 통과 (닉네임 변경 테스트 3건 추가: 변경 후 `me` 반영 · 빈 값과 51자 400 · 토큰 없이 401)
- [x] 로그인 · 대시보드 · 내역(검색 결과 없음 상태) · 예산·설정 · 내역관리 화면이 실제 브라우저(Playwright)에서 정상 렌더링됨. 콘솔 오류 없음
- [x] 다크 모드 대시보드(히어로·요약·히트맵) 렌더링 확인
- [x] 닉네임 변경이 실제 화면에서 동작함: 팝업 → 저장 → 헤더·대시보드 인사말 즉시 반영, 새로고침 후에도 유지(서버 저장 확인), 원래 값으로 복구
- [x] 카테고리별 지출 차트 세 가지, 달력 이전·다음 달 날짜, 모바일(390px) 로그인 배너, 챗봇 이동·크기 조절이 브라우저(Playwright)에서 동작함
- [ ] 미실행: 위 변경 이후의 `npm run build`(타입·린트는 통과, 빌드는 다시 돌리지 않음)
- [ ] 미검증: 새 그라데이션·달력 이전·다음 달 칸·차트 세 종류의 다크 모드, 실제 터치 기기에서의 챗봇 드래그, 세로가 짧은 폰(예: 667px)의 모바일 로그인 배치
- [ ] 미해결: JSON 본문의 문자 인코딩이 깨진 요청이 400이 아니라 500(`INTERNAL_ERROR`)으로 나간다(`GlobalExceptionHandler`에 `HttpMessageNotReadableException` 매핑 없음). 브라우저는 UTF-8로 보내 평소엔 드러나지 않는다
- [ ] 미검증: 다크 모드의 로그인 화면 · 다크 히트맵(칸이 라이트 팔레트 그대로라 밝게 보임)
- [ ] 미검증: 실제 터치 기기의 44px 타겟, 스크린 리더(NVDA 등) 읽기 순서, OS의 "동작 줄이기" 실제 설정
- [ ] 미검증: 거래가 0건인 계정의 `LedgerArt` 빈 상태, 네트워크 오류 시 `ErrorArt`, 토스트 다크 전환(`sonner.tsx` 수정 후)
