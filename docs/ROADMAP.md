# ROADMAP — 머니로그(MoneyLog)

> **버전** 1.2 · **최종 수정** 2026-09-16
...
- [ ] `/login`의 **회원가입 링크로 `/signup`에 도달**함 (URL 직접 입력 없이 — `PRD.md` 5.2)
- [ ] 가입 → 자동 로그인 → `/dashboard` 이동
- [ ] 가입 직후 **`GET /api/v1/categories`가 9개를 반환**하고, 그 토큰이 새로 가입한 계정의 것임
...
- [ ] **360px에서 네비게이션이 하단 탭 바로 바뀌고**, 데스크톱에서는 헤더 안에 가로로 배치됨 (`PRD.md` 5.1)
- [ ] 모든 입력에 label이 연결됨 (⚠️ `placeholder`를 label 대용으로 쓰지 않았는지 함께 확인 — `UX-06`)
- [ ] `npm run build` 통과
...
- [ ] 거래 0건일 때와 검색 결과 0건일 때 **문구가 다르고**, 각각의 CTA가 동작함 — 전자는 **금액 입력에 포커스가 들어가고**, 후자는 **필터 초기화 버튼이 URL 쿼리를 비움**
- [ ] 목록 로딩 중 **스피너가 아니라 항목형 스켈레톤**이 보임 (⚠️ 로컬 API는 빨라 잘 안 보인다. 네트워크 스로틀링을 켜고 확인 — `UX-01`)
- [ ] 에러 상태의 **재시도 버튼이 실제로 재요청**함 (네트워크 탭 확인)
- [ ] 카테고리 배지에서 **이름 텍스트가 카테고리 색 위에 얹혀 있지 않음** (색은 점·막대로 옆에 — `PRD.md` 5.1)
...
- [ ] 고정지출 카드가 시드에 심어둔 "넷플릭스"·"통신비"를 찾아내고, **연속 개월 수·최근 날짜·월 합계가 모두 표시됨**
- [ ] 고정지출 카드가 **본문보다 늦게 로드되어도 레이아웃이 밀리지 않음** (로딩 중 스켈레톤이 최종 높이를 차지 — `UX-01`)
- [ ] 도넛 비율 합이 100%로 보임
- [ ] 히트맵의 1일이 **올바른 요일 칸에 위치**함 (여러 달을 이동하며 확인)
- [ ] 히트맵의 각 칸에서 **날짜와 금액을 읽을 수 있음** (툴팁 또는 보조 텍스트 — `PRD.md` 5.4)
- [ ] 히트맵에 **그 달의 모든 날 칸이 존재**함 (지출 0원인 날도 칸이 있고, 비는 칸은 1일 앞의 요일 오프셋뿐 — `CLAUDE.md` 5장)
- [ ] 대시보드 로딩 중 **카드형 스켈레톤**이 보임 (스피너 없음 — `UX-01`)
- [ ] 백엔드를 중지한 채 진입하면 에러 카드가 뜨고, **재시도 버튼이 실제로 재요청**함 (백엔드를 다시 켜고 누르면 화면이 복구됨 — `UX-04`)
...
- [ ] 라이트·다크 양쪽에서 차트가 읽힘 — **카테고리 색이 양쪽에서 동일하고**, 축·범례 텍스트가 카테고리 색 위에 올라가 있지 않음 (`PRD.md` 5.1)
- [ ] 360px에서 차트가 깨지지 않음 (히트맵 7열이 유지되고 가로 스크롤이 생기지 않음)
- [ ] `npm run build` 통과
...
- [ ] 카테고리를 추가하면 **퀵 입력 바의 카테고리 목록에 즉시 나타남**(`['categories']` 무효화), 이름·색·표시 순서를 수정하면 목록 순서와 배지 색이 그에 맞게 바뀜 — `CAT-01`·`CAT-02`
- [ ] 같은 구분 안에서 중복 이름 저장 시 **이름 입력 아래 인라인 안내**, **삭제한 이름은 다시 저장됨** — `CAT-04`
- [ ] 카테고리 삭제 확인 문구에 "과거 내역은 그대로 남습니다"가 포함됨 — `CAT-03`
...
- [ ] `/data`에 **형식 안내 표 · 상한 안내(1MB / 5,000행) · 중복 등록 안내 문구**가 모두 보임 (`PRD.md` 5.9)
...
- [ ] **Swagger UI에 엔드포인트 18개가 모두 노출**됨 — 인증 3 · 카테고리 4 · 거래 5 · 집계 2 · 예산 2 · 데이터 2 (`CLAUDE.md` 5장이 정본). Authorize 버튼에 토큰을 넣고 보호된 API가 실제로 호출됨
  > ⚠️ Phase 1·3에서 확인한 시점 이후 **API가 12개 늘었다.** 특히 `GET /data/export`는 `ApiResponse` 봉투 예외라 응답 스키마가 다르고, `POST /data/import`는 multipart라 Swagger가 파일 업로드 폼을 따로 그린다 — 이 둘이 누락되거나 잘못 그려지기 쉽다
...
- [ ] 3회 조작으로 거래 1건 등록 — `TXN-01`
- [ ] **최근 사용 카테고리 3개 버튼이 실제 최근 사용 순으로 노출**되고, 그 버튼이 위 3회 조작의 두 번째 조작임 — `TXN-03`
- [ ] 키보드만으로 거래 1건 등록 완주 — `UX-06`
- [ ] 같은 날짜 거래 다수에서 페이지 경계 중복·누락 없음 — `TXN-06`
- [ ] **필터(기간·구분·카테고리)와 키워드 검색이 URL에 반영**되고, 그 URL을 새 탭에 붙여넣으면 같은 결과가 나옴 — `TXN-07`·`TXN-08`
- [ ] **거래 삭제가 클릭 즉시 목록에서 사라지고**, 서버를 중지한 상태로 삭제하면 **항목이 되돌아오며 토스트가 뜸** — `TXN-10`·`TXN-11`
- [ ] 카테고리 삭제 후 과거 내역 보존 + "삭제됨" 표시 — `CAT-03`
- [ ] 삭제한 카테고리 이름 재사용 가능 — `CAT-04`
- [ ] 타인 거래 접근 시 404 전용 화면 — `TXN-12`
- [ ] 이탈 확인 3경로 전부 동작, 저장 후에는 뜨지 않음 — `TXN-09`

**D. 집계 · 예측 (STAT-01~08, BUD-03·04)**

- [ ] `grep -r "now()"` 결과가 `StatsService`에 없음
- [ ] **월 이동이 URL에 반영되고 뒤로가기로 이전 달로 돌아감**, 미래 달로는 이동할 수 없음 — `STAT-01`
- [ ] 거래 없는 달이 오류 없이 0으로 표시 — `STAT-08`
- [ ] 예측 금액이 서버 응답과 일치 (프론트 재계산 없음) — `STAT-04`
- [ ] 고정지출 감지가 시드의 3개월 연속 항목을 찾아내고 **월 합계가 표시됨** — `STAT-06`
- [ ] 히트맵 1일 요일 정렬이 여러 달에서 정확하고, **각 칸의 날짜·금액을 읽을 수 있음** — `STAT-03`
- [ ] 예산 0 카테고리가 `NaN`/`Infinity`로 표시되지 않음 — `BUD-03`
- [ ] 거래 추가 후 대시보드 숫자가 갱신됨

**E. CSV (CSV-01~07)**

- [ ] 내보낸 파일이 **Microsoft Excel에서 한글 깨짐 없이** 열림
- [ ] 콤마·따옴표·줄바꿈이 든 메모의 왕복 일치 — `CSV-05`
- [ ] 부분 실패 시 **화면에** 행 번호와 사유가 표시되고, 성공분은 목록에 등록되어 있음 — `CSV-04`
...
- [ ] 모든 로딩이 스켈레톤 (스피너 없음). **네트워크 스로틀링을 켜고** 대시보드·목록·상세 세 화면에서 확인 — `UX-01`
- [ ] 빈 상태 / 검색 결과 없음 문구 구분, **각각의 CTA(포커스 이동 / 필터 초기화)가 동작** — `UX-02`·`UX-03`
- [ ] 모든 에러 상태의 재시도 버튼이 실제 재요청 — `UX-04`
- [ ] **360px · 1920px** 양쪽에서 레이아웃 정상. 360px에서 **가로 스크롤이 생기지 않고** 네비게이션이 하단 탭 바임 — `UX-05`
- [ ] 라이트·다크 양쪽에서 **8개 화면 전부** 확인(`/login`·`/signup`·`/dashboard`·`/transactions`·`/transactions/[id]`·`/budgets`·`/settings/categories`·`/data`). 카테고리 색 위에 텍스트가 얹힌 곳이 없음 — `UX-07`
- [ ] 목록 금액 `tabular-nums` 정렬 — `UX-08`
